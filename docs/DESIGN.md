...
- **§25 Phase 1 "Memory HLE" closed** (SetHeapSize, MapMemory, QueryMemory - the checkbox's own title - plus UnmapMemory, MapMemory's inverse, named alongside the other three in §12's priority list). New `core/hle/kernel/svc_memory.{h,c}`, header-reviewed before implementation. `HLE_Context` (`hle.h`) gains `vmm`/`process`/`pages` pointers - the same "environment bundle" shape `Thread_Env` already used for thread creation - so the four handlers stay plain functions over data they don't own; `hle_context_init` and `process_teardown` (now also releasing the heap and unwinding live borrows) both gained a parameter, a small, mechanical ripple into `emulator.c` and three test files.
- **Register ABI verified against libnx's `svc.s`/`svc.h`** (the public SDK every title links, not another emulator's implementation) rather than assumed from C parameter order, which the dispatch table's own comment does NOT hold for these four - two have an out-pointer first C parameter that the kernel never actually sees. Correction found this way: **SetHeapSize's requested size arrives in X1, not X0** - libnx's raw stub leaves X0 untouched by the syscall itself, only using it locally afterward to store the kernel's returned heap-base register into the caller's out-pointer. QueryMemory: X0 is the *guest* address the kernel writes the `MemoryInfo` struct into, X2 is the query address, X1 is dead the same way SetHeapSize's X0 is.
- **MapMemory/UnmapMemory semantics corroborated against real usage**, not just the kernel's stated capability: switchbrew/libnx document that the source range is reprotected to `Perm_None` (mirror, not move) and that a partial unmap is kernel-legal, but libnx's actual `thread.c` - the only real-world caller (`threadCreate`/`threadClose`, backing a thread's stack-in-heap trick) - always maps and unmaps the *exact same* `(dst, src, size)` triple it stored. Voland's UnmapMemory requires that exact match; stated as a deviation in `svc_memory.h` rather than implemented against the kernel's full generality, since nothing exercises it.
- **`vmm_guest_to_host` + `layout_get()->guest_ram_base`, not a new vmm accessor**, recovers the guest-physical address MapMemory needs to alias and SetHeapSize's shrink needs to release - vmm's PTE format deliberately erases `guest_pa` into a host offset at map time (§5), and adding a reverse accessor for one caller was rejected in favor of reusing the sanctioned "get a host pointer, borrow-scoped" API already used everywhere else.
- **`page_allocator.{h,c}` gained release** (`page_allocator_free`, a small freelist): the first real caller of the release path its own header already anticipated. First-fit allocation, checking the freelist before bumping; a free adjacent to the bump cursor retracts it directly rather than entering the freelist (so a shrink-then-regrow of the same tail costs nothing); coalescing elsewhere is deliberately minimal - the cursor and at most one existing neighbor, not a general merge - stated as sized to Phase 1's one shrinking consumer, not a general-purpose allocator.
- **A dispatch-wide borrow scope was tried first and rejected during testing**, not shipped: bracketing all of `hle_on_svc` in one `vmm_borrow_scope_begin/end` pair (as `vmm.h`'s own comment on the pattern suggested) immediately hit `vmm.c`'s debug borrow cap (`VMM_DEBUG_MAX_BORROWS` = 64) - SetHeapSize's shrink path can call `vmm_guest_to_host` once per released page, hundreds of times in one dispatch, and the cap exists to catch a *leaking* handler, not to size a loop. Fixed by scoping each `vmm_guest_to_host` call individually instead of dispatch-wide: a fast path tries the whole range as one physical run first (the common case - one grow is one `page_allocator_allocate` run in one `vmm_map` call), falling back to one page at a time, each in its own scope, only when the backing is fragmented.
- **QueryMemory's `MemoryInfo.type`/`attr` fidelity is deliberately partial, stated in `svc_memory.h`**: Horizon's real `MemoryType` has ~30 values; Voland reports only the five address-space-region buckets (code/heap/stack/TLS/unmapped) reachable by a title before the SVCs that create the other kinds (transfer memory, IPC buffers, module code, ...) exist - a closure argument over Phase 1, not an unexamined gap, using Horizon's own numeric encoding for the values it does model (`HLE_MEMTYPE_HEAP=0x05`, `HLE_MEMTYPE_MAPPED_MEMORY=0x0B`, etc., from libnx's enum) so a title decoding them raw is not lied to. `attr` (e.g. `MemAttr_IsBorrowed`) is always 0 - the observable behavior (inaccessible) already comes through `perm` exactly.
- **`HLE_RESULT_OUT_OF_MEMORY` corrected** from `0x1A01` (module 13, no matching `KernelError_*` - a Phase 0 placeholder never actually returned to a guest register until now) to `0xD001` (`KernelError_OutOfMemory` = 104), verified against libnx's `result.h`. Three constants added for the same reason, same source: `HLE_RESULT_INVALID_SIZE` (0xCA01, 101), `HLE_RESULT_INVALID_MEMORY_STATE` (0xD401, 106, aka `InvalidCurrentMemory`), `HLE_RESULT_INVALID_MEMORY_RANGE` (0xDC01, 110).
- **Unrelated fix surfaced by a `-Werror` pass:** a stray `/*` inside a `romfs.c` doc comment (pre-existing, predates this change) tripped `-Wcomment` under GCC `-Werror`; reworded, no behavior change.
- Tests: new `tests/svc_memory_test.c` (SVC-path style, like `thread_test.c` - a real `process_bootstrap()` over a synthesized one-module ExeFS, `hle_on_svc` driven directly, state asserted through the CPU backend and vmm) covering heap grow/shrink/regrow with granule-alignment and region-capacity rejection, MapMemory/UnmapMemory's happy path plus every rejection path plus borrow-table exhaustion plus exact-match enforcement, QueryMemory across every modeled region, and a scenario that deliberately forces `page_allocator`'s freelist path (not just cursor retraction) by interleaving an unrelated allocation the way Phase 2's `CreateThread` eventually will. `tests/page_allocator_test.c` extended with release/coalesce/split/exhaustion coverage. Mechanical call-site updates in `emulator.c`, `process_test.c`, `thread_test.c`, `smoke_phase0.c` for the two changed signatures and two SVC ids those tests used as "anything unimplemented" stand-ins that are no longer unimplemented (0x01 → 0x02).
- **Verified:** MSVC (VS 2026 Build Tools, `cl` via `Launch-VsDevShell.ps1`) and GCC 16.2.1 (WSL), 16/16 ctest on both; zero diagnostics at MSVC `/W4 /WX` and GCC `-Werror` (the romfs.c fix above was needed to get there). Mutation-checked: disabling the shrink-vs-live-borrow guard aborts on an internal vmm invariant (attempting to read a page still reprotected to `VMM_PERM_NONE`) rather than corrupting state silently; disabling UnmapMemory's exact-match check aborts inside `vmm_unmap` the same way; disabling `page_allocator`'s cursor-retraction branch is caught directly by `page_allocator_test`'s `used_bytes` assertion. All three reverted and reverified clean afterward.
